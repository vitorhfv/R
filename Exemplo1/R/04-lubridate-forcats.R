...
library(lubridate)
dif <- dmy("15/04/2021") - dmy("24/08/1143")
dif <- now() - dmy_hms("20/10/2022 19:36:00")

as.numeric(as.period(dif))
...
imdb10 <- readr::read_rds("data/imdb.rds")
...
imdb10 |>
  count(classificacao) |>
  mutate(classificacao_fator = fct_relevel(
    classificacao,
    c("Livre",
      "A partir de 18 anos",
      "A partir de 13 anos",
...
imdb10 %>%
  tidyr::separate_rows(generos, sep = "\\|") %>%
  mutate(generos = fct_lump(generos, n = 50)) %>%
  count(generos) %>%
  arrange(n)
...
imdb %>%
  tidyr::separate_rows(generos, sep = "\\|") %>%
  mutate(generos = fct_lump_min(generos, min = 100)) %>%
  grafico_lucro_medio()
